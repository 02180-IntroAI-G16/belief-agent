...
const BeliefQuery = ({ handleQueryBelief }) => {
  const inputRef = useRef(null);
  const [belief, setbelief] = useState("");

  const addSymbol = ({ symbol }) => {
    if (!belief.endsWith(symbol)) {
      setbelief((prev) => prev + symbol);
  ...
  return (
    <div className="bg-slate-50 rounded-[10px] p-4">
      <div className="bg-gray-600 text-gray-50 rounded p-2 font-semibold w-fit">
        Check if we believe the following:
      </div>
